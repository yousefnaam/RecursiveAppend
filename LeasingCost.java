public class LeasingCost {
	...
	public static Vehicle createVehicle(String description){
        ...
        int name = description.indexOf("name");
        String s = description.substring(name+5);
        String newName = s.substring(0,s.indexOf(";"));

        Fuel fuel;

        if(description.indexOf("charger:") != -1) {
        int charge1 = description.indexOf("charger:");
        String end = description.substring(charge1+8);
        String charge2 = end.substring(0,end.indexOf(";"));
        chargerCost = Double.parseDouble(charge2);
        }


        int iDues = description.indexOf("due:");
        String duesIndex = description.substring(iDues+4);
        String endDue = duesIndex.substring(0,duesIndex.indexOf(";"));
        double due = Double.parseDouble(endDue);
    
         int iLength = description.indexOf("length:");
         String l1 = description.substring(iLength+7);
         String lengthFinal = l1.substring(0,l1.indexOf(";"));
         int length = Integer.parseInt(lengthFinal);
      
        int iMonth = description.indexOf("monthly:");
        String m2 = description.substring(iMonth+8);
        String monthFinal = m2.substring(0,m2.indexOf(";"));
        int month = Integer.parseInt(monthFinal);
      
        int mpu1 = description.indexOf("mile/unit:");
        String mpufirst = description.substring(mpu1+10);
        String mpuFinal = mpufirst.substring(0,mpufirst.indexOf(";"));
        double miles = Double.parseDouble(mpuFinal);
      
        int allowance = description.indexOf("allowance:");
        String allowancetemp = description.substring(allowance+10);
        String allowanceFinal = allowancetemp.substring(0,allowancetemp.indexOf(";"));
        int mileageAllowance = Integer.parseInt(allowanceFinal);
        ...
        if (description.indexOf("charger:") == -1) {
            ...
            fuel = new Fuel(miles,chargerCost);
        }
     
        
      Lease lease = new Lease(due,length,month,mileageAllowance);
        Vehicle v = new Vehicle(newName,fuel,lease);
        return v;
	}
     ...
     */
        ...
        public static void computeCO2EmissionsAndCost( Vehicle[] vehicles, double gasPrice, double electricityPrice ){
            ...
            for (int i = 0; i < vehicles.length; i++) {
                ...
                double months = tempV.getLease().getLeaseLength();
                double usage = tempV.getFuel().getUsage();
                double mileageAllowance = tempV.getLease().getMileageAllowance();
                double co2PerUnit = 0;
                double due = tempV.getLease().getDueAtSigning();
                double monthlyCost = tempV.getLease().getMonthlyCost();
                double leaseCost = computeLeaseCost(due, months, monthlyCost);
                if (tempV.getFuel().getType() == 1) {
                    co2PerUnit = Fuel.CO2EMITTED_GASCOMBUSTION;
                    tempV.setFuelCost(computeFuelCost(months, usage, mileageAllowance, gasPrice));
                } else {
                    co2PerUnit = Fuel.CO2EMITTED_ELECTRICITYCOMBUSTION;
                    tempV.setFuelCost(computeFuelCost(months, usage, mileageAllowance, electricityPrice));
                }
                double total = tempV.getFuelCost() + leaseCost;
                tempV.setTotalCost(total);
                tempV.setCO2Emission(computeCO2(months, usage, mileageAllowance, co2PerUnit));
            }
        }
